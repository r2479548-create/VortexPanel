import os, secrets
from panel.utils.shell import run_ok

def provision_smm_panel(domain):
    # 1. Create directory structure for the domain
    site_dir = f"/var/www/{domain}"
    run_ok(f"mkdir -p {site_dir}/public_html")
    run_ok(f"mkdir -p {site_dir}/logs")
    ...
    if os.path.exists(script_zip):
        run_ok(f"unzip -q {script_zip} -d {site_dir}/public_html/")
    else:
        # Create a dummy index.html if zip doesn't exist for testing
        run_ok(f"echo '<h1>SMM Panel Provisioned for {domain}!</h1>' > {site_dir}/public_html/index.html")
    ...
    # 4. Set permissions
    run_ok(f"chown -R www-data:www-data {site_dir}")
